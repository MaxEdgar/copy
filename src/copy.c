 ...
 *
 * Works on X11, Wayland, WSL, macOS, and Termux.
 *
 * Usage:
 *   echo hello | copy
 *   cat file.log | copy
...
int main(int argc, char **argv) {
    ...
    if (which("termux-clipboard-set")) {
        candidates[nc].bin = "termux-clipboard-set";
        candidates[nc].args[0] = "termux-clipboard-set";
        candidates[nc].args[1] = NULL;
        nc++;
    }
    ...
    if (nc == 0) {
        struct { const char *bin; const char *a1, *a2; } fb[] = {
            ...
            {"clip.exe", NULL, NULL},
            {"pbcopy", NULL, NULL},
            {"termux-clipboard-set", NULL, NULL},
        };
