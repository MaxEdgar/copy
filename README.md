...
automatically detecting whether to use an X11, Wayland, WSL, macOS, or
Termux clipboard backend. It has no runtime dependencies of its own; it
...
- Automatic backend detection: X11 (`xclip`, `xsel`), Wayland (`wl-copy`),
  WSL (`clip.exe`), macOS (`pbcopy`), and Termux (`termux-clipboard-set`)
...
macOS, and Termux clipboard backends.
